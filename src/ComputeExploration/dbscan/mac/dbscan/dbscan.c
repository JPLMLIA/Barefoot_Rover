...
/* Add an index node to linked list of neighbors. */
int append_at_end(
     unsigned int index,
     epsilon_neighbours_t *en)
{
...
/* Return the indicies of all neighbors in a linked list structure. */
epsilon_neighbours_t *get_epsilon_neighbours(
    unsigned int index,
	unsigned int num_features,
    double points[][num_features],
...
/* Iteratively try to create a new cluster from the next unclassified point.
 * Once all points have been classified, we are done. */
void dbscan(
	unsigned int num_features,
    double points[][num_features],
	int labels[],
...
/* Attempt to begin a new cluster at UNCLASSIFIED point `index` (check for >= `minpts` neighbors).
 * If successful, call spread on neighbors (seeds) until edges of new cluster reached. 
 * Otherwise, label this point NOISE (may still be found to be an EDGE of some later cluster). */
int expand(
    unsigned int index,
    unsigned int cluster_id,
	unsigned int num_features,
...
/* Find all epsilon neighbors of seed point `index`. 
 * If number exceeds minpts, label all neighbors the current cluster,
 * and add any unclassified neighbors as seeds for this cluster to later spread. */
int spread(
    unsigned int index,
    epsilon_neighbours_t *seeds,
    unsigned int cluster_id,
...
/* Read in metadata (parameters, number of features and points). */
unsigned int parse_input1(
    FILE *file,
    double *epsilon,
    unsigned int *minpts,
...
/* Read in parameters and point data, then run dbscan. */
int main(void) {
    double epsilon;
    unsigned int minpts;
    unsigned int num_points =
