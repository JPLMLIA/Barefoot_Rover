...
kdtree2::kdtree2(kdtree2 * other)
  : the_data(other->the_data),
    N(other->N),
    dim(other->dim),
    sort_results(other->sort_results),
    rearrange(other->rearrange),
    root(other->root),
    data(other->data),
    ind(other->ind),
    bucketsize(other->bucketsize),
    rearranged_data(other->rearranged_data)
{ 
  if(other->rearrange) {
    cout << "Rearrange on copy not implemented" << endl;
    exit(1);
  }
}

// constructor
kdtree2::kdtree2(array2dfloat& data_in,int num_points, int dims, int bucketsize, bool rearrange_in,int dim_in)
  : the_data(data_in),
//    N  ( data_in.shape()[0] ),
//    dim( data_in.shape()[1] ),
#if 0
...
#else
    N (num_points), //N  ( data_in.shape()[0] ),
    dim (dims), //dim( data_in.shape()[1] ),
#endif
    sort_results(false),
...
//shallow copy
kdtree2_node::kdtree2_node(kdtree2_node * other) : box(other->box){
...
void kdtree2_node::search(searchrecord& sr) {
  ...
  if ( (left == NULL) && (right == NULL)) {
    ...
    //    cout << tid << "\tTerminal Node!" << endl;
    if (sr.nn == 0) {
      process_terminal_node_fixedball(sr);
    } else {
      process_terminal_node(sr);
    }
    //    cout << "\tFinished Processing terminal node" << endl;
  } else {
