...
namespace NWUClustering
{
	...
	int Clusters::read_file(char* infilename, int isBinaryFile)
	{
        ...
        int Clusters::build_kdtree(int bucketsize, bool use_simd)
	{
		if(m_pts == NULL)
		{
		...
		m_kdtree = new kdtree2(m_pts->m_points, bucketsize, use_simd, false);
        ...
        int Clusters::build_kdtree_outer(int bucketsize, bool use_simd)
	{
		if(m_pts_outer == NULL)
		{
		...
		if(m_pts_outer->m_i_num_points > 0)
		{
		  ...
		  m_kdtree_outer = new kdtree2(m_pts_outer->m_points, bucketsize, use_simd, false);
