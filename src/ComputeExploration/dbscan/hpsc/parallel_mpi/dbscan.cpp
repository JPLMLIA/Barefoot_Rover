...
namespace NWUClustering
{
	...
	void run_dbscan_algo_uf_mpi_interleaved(ClusteringAlgo& dbs)
	{
		...
		vector < vector <int > > merge_received;
		vector<int> merge_send1[nproc];
		vector<int> merge_send2[nproc];
		vector <int> init;
		...
		for(pid = 0; pid < nproc; pid++)
		{
			...
			merge_send1[pid] = vector<int>(dbs.m_pts->m_i_num_points * nproc);
			merge_send2[pid] = vector<int>(dbs.m_pts->m_i_num_points * nproc);
		}

		int root, root1, root2, tid;

		vector <int >* pswap;
		vector <int >* p_cur_send;
		vector <int >* p_cur_insert;
		...
		while(1)
		{
			pswap = p_cur_insert;
            		...
            		for(tid = 0; tid < nproc; tid++) {
			       vector<int>().swap(p_cur_insert[tid]);
			}
			...
			for(tid = 0; tid < nproc; tid++)
			{
				...
				{
				  merge_received[tid].clear();
				  vector<int>().swap(merge_received[tid]);
					merge_received[tid].assign(irecv[tid], -1);
					MPI_Irecv(&merge_received[tid][0], irecv[tid], MPI_INT, tid, tag + 1, MPI_COMM_WORLD, &d_req_recv[rcount]);
						rcount++;
				}
			...
			for(tid = 0; tid < rcount; tid++)
			{
				...
				if(rtag == tag + 1)	
				{
					...
					vector<int>().swap(merge_received[rsource]);
		...
		//Data owned by merge_send vecs, this does not leak
		pswap = NULL;
		p_cur_insert = NULL;
		p_cur_send = NULL;
		...
		{
		        vector<int>().swap(merge_received[tid]);
			vector<int>().swap(merge_send1[tid]);
			vector<int>().swap(merge_send2[tid]);
		}
