...
static void usage(char *argv0) 
{
	...
	"Usage: %s [switches] -i filename -b -m minpts -e epsilon -o output\n"
        ...
        "       -c bucketsize   : hyperparameter of kdtree, minimum number of points required to split a node\n"
...
int main(int argc, char** argv)
{
	...
	infilename	= NULL;
    	...
    	while ((opt=getopt(argc,argv,"i:m:e:o:c:s:?b"))!= EOF)
    	{
		switch (opt)
        	{
        		case 'i':
	...
	start = MPI_Wtime();
	dbs.build_kdtree(bucketsize, use_simd);
	dbs.build_kdtree_outer(bucketsize, use_simd);
	MPI_Barrier(MPI_COMM_WORLD);
