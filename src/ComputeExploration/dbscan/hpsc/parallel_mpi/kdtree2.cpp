...
// constructor
kdtree2::kdtree2(array2dfloat& data_in,int bs,bool simd,bool rearrange_in,int dim_in)
  : the_data(data_in),
    //N  ( data_in.shape()[0] ),
    //dim( data_in.shape()[1] ),
    N (data_in.size()),
...
void kdtree2_node::process_terminal_node(searchrecord& sr) {
  ...
  for (int i=l; i<=u;i++) {
    ...
    if (rearrange) {
      ...
      if (use_simd) {
#if SIMD_CHECKFREQ
	simd_squared_dist(data[i].data(), sr.qv.data(), dim, &dis, ballsize, SIMD_CHECKFREQ);
#if _DEBUG_SIMD_UTILS_H
	dis1 = 0.0;
	simd_squared_dist(data[i].data(), sr.qv.data(), dim, &dis1);
	if((dis < ballsize || dis1 < ballsize) && dis != dis1) {
	  printf("Error: Eearly termination result did not match actual distance:\n\tActual = %f\n\tEarly = %f\n\tEps = %f\n",
		 dis1, dis, ballsize);
	  exit(1);
	}
#endif
#else
	simd_squared_dist(data[i].data(), sr.qv.data(), dim, &dis);
#endif
	if (dis > ballsize) {
	  early_exit=true;
	}            
	...
	for (int k=0; k<dim; k++) {
      ...
      if(use_simd) {
#if SIMD_CHECKFREQ
	simd_squared_dist(data[indexofi].data(), sr.qv.data(), dim, &dis, ballsize, SIMD_CHECKFREQ);
#if _DEBUG_SIMD_UTILS_H
	dis1 = 0.0;
	simd_squared_dist(data[indexofi].data(), sr.qv.data(), dim, &dis1);
	if((dis < ballsize || dis1 < ballsize) && dis != dis1) {
	  printf("Error: Eearly termination result did not match actual distance:\n\tActual = %f\n\tEarly = %f\n\tEps = %f\n",
		 dis1, dis, ballsize);
	  exit(1);
...
#endif
#else
	simd_squared_dist(data[indexofi].data(), sr.qv.data(), dim, &dis);
#endif
	if (dis > ballsize) {
	  early_exit=true;
	}            
